...
if __name__ == "__main__":
    ...
    try:
        ...
        imagesDict = _wmts.call_IGN_WMTS_bbox(ne_x, ne_y, sw_x, sw_y, lvl, tileMatrixSet.getIdentifier())
        for x in imagesDict:
            for y in imagesDict[x]:
                imagesDict[x][y].save(f"/tmp/outputDict_{x}_{y}.png")
    except Exception as err:
        print(f"Unexpected {err=}, {type(err)=}")
